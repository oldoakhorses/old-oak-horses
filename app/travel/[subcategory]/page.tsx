...
export default function TravelSubcategoryPage() {
  ...
  const bills: any[] = useQuery(api.bills.getTravelBills, travelCategory ? { categoryId: travelCategory._id } : "skip") ?? [];

  const [search, setSearch] = useState("");
  const [page, setPage] = useState(1);

  const subBills: any[] = useMemo(() => bills.filter((bill) => (bill.travelSubcategory || "") === subcategory), [bills, subcategory]);

  const filtered: any[] = useMemo(() => {
    const q = search.trim().toLowerCase();
    if (!q) return subBills;
    return subBills.filter((bill) => {
  ...
  return (
    <div className="page-shell">
      ...
      <main className="page-main">
        <Link href="/travel" className="ui-back-link">
          ← cd /travel
        </Link>

        <section className={styles.headerCard}>
          <div className="ui-label">TRAVEL · {subcategory.toUpperCase()}</div>
